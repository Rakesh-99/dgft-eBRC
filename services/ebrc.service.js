...
// Step 4: AES 256 bits key is generated by salting secret key from step 3 with 32 bytes characters using SHA 256

function createAES256Key(secretKey, salt) {
    const key = Buffer.concat([
        Buffer.from(secretKey, 'utf8'), // 32-byte ASCII string → 32 bytes
        salt                            // 32 random bytes
    ]);
    return crypto.createHash('sha256').update(key).digest(); // 32-byte AES key
}
...
function encryptAESKey(secretKey) {
    try {
        ...
        console.log("Secret key string:", secretKey);
        ...
        console.log("- Input: 32 bytes");
        console.log("- Output: ", encryptedKey.length, "bytes");
        console.log("- Base64:", encryptedKeyBase64.length, "chars");
        console.log("- Base64 preview:", encryptedKeyBase64.substring(0, 60), "...");
...
async function encryptPayload(payload) {
    ...
    // Step 3: Generate 32-char secret key (FIXED)
    ...
    if (localDecrypt !== payloadBase64) {
        throw new Error("LOCAL DECRYPTION FAILED: The encrypted data cannot be decrypted back to original Base64 payload.");
    }
